...
public class BallGame{
    ...
    public void setupGUI(){
        UI.setMouseMotionListener(this::launch);         // the mouse will launch the ball
    ...
    public void launch(String action, double x, double y){
        if (action.equals("released") == false && gameFin == false){
            Color lineColor = this.ball.returnColor();
            UI.setColor(lineColor);
            UI.drawLine(LAUNCHER_X,GROUND-LAUNCHER_HEIGHT, x, y);
        }
    ...
    public void runGameTwoTargets(){
        gameFin = false;
        UI.printMessage("Click Mouse to launch the ball");
        this.ball = new Ball(LAUNCHER_X, LAUNCHER_HEIGHT); // makes shooter ball
        double shelfHeight1 = 50+Math.random()*400;
        Ball target1 = new Ball(SHELF_X, shelfHeight1); // makes target one
        double shelfHeight2 = 50+Math.random()*400;
        Ball target2 = new Ball(SHELF_X, shelfHeight2); // makes target two
        this.drawGameTwoTargets(this.ball, target1, target2, shelfHeight1, shelfHeight2);
        int count = 0; 
        
        // run until both targets are gone (ie, off the right end)
        while ((target1.getX()<RIGHT_END) || (target2.getX()<RIGHT_END)) {
            //if the ball is over the right end, make a new one.
            if (ball.getX()>=RIGHT_END) {
                this.ball = new Ball(LAUNCHER_X, LAUNCHER_HEIGHT);
                count++;
